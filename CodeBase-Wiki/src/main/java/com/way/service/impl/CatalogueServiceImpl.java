...
import com.way.llm.prompt.AnalyzeCataloguePrompt;
...
import com.way.utils.RegexUtil;
...
public class CatalogueServiceImpl extends ServiceImpl<CatalogueMapper, Catalogue> implements ICatalogueService {
    ...
    private List<Catalogue> saveCatalogueItem(ExecutionContext context, CatalogueStruct.Item item, String parentCatalogueId) {
        ...
        catalogueEntity.setRepoId(context.getTask().getProjectName());
    ...
    public void parallelGenerateCatalogueDetail(String fileTree, GenCatalogueDTO genCatalogueDTO, String localPath, String projectName) {
        ...
        // 在发送任务前，先设置任务总数到Redis
        if (!cataloguesToProcess.isEmpty()) {
            String taskId = cataloguesToProcess.get(0).getTaskId(); // 所有目录都属于同一个任务
            taskProgressService.setTaskTotal(taskId, cataloguesToProcess.size());
            log.info("已设置任务总数到Redis: taskId={}, totalCount={}", taskId, cataloguesToProcess.size());
        }
