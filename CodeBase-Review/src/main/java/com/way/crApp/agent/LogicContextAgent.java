...
/**
 * 逻辑与上下文审查员Agent - 集成Wiki服务检查代码逻辑和设计模式
 * <p>
 * 职责：
 * 1. 通过Wiki服务获取项目上下文
...
public class LogicContextAgent implements NodeAction {
    ...
    private ReviewESService reviewESService;
    ...
    private String retrieveProjectContext(String repoName, String diffContent,
        ...
        try {
            if (reviewESService == null) {
                logger.warn("Wiki服务不可用，跳过上下文检索");
                return null;
            }

            // 先检查Wiki服务是否可用
            if (!reviewESService.isWikiServiceAvailable()) {
                logger.warn("Wiki服务当前不可访问，将使用降级模式");
                return buildFallbackContext(repoName, prTitle, changedFiles);
            }

            logger.info("调用Wiki服务获取项目上下文: {}", repoName);
            ...
            while (context == null && retryCount < maxRetries) {
                try {
                    context = reviewESService.searchContextForCodeReview(
                            repoName,
                            codeSnippet,
                            prTitle,
                            "检查代码逻辑和设计模式一致性",
                    ...
                    if (retryCount >= maxRetries) {
                        logger.error("Wiki服务调用失败，已重试{}次", retryCount, innerEx);
                        return buildFallbackContext(repoName, prTitle, changedFiles);
                    }
                    logger.warn("Wiki服务调用失败，重试 {}/{}", retryCount, maxRetries);
    ...
    private String buildFallbackContext(String repoName, String prTitle, String[] changedFiles) {
        ...
        context.append("\n注意：Wiki服务暂时不可用，基于代码变更内容进行分析。\n");
        context.append("建议：\n");
        context.append("1. 确保Wiki服务已启动（端口8080）\n");
